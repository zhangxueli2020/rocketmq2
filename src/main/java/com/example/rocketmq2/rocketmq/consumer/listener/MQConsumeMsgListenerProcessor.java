package com.example.rocketmq2.rocketmq.consumer.listener;
...
//	private TransactionMQProducer transactionMQProducer;
    ...
    public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> msgs, ConsumeConcurrentlyContext context) {
        ...
        if (messageExt.getTopic().equals("Db2Topic")) {
            if (messageExt.getTags().equals("tagRollback2")) {
                ...
                String s = null;
                try {
                    s = new String(messageExt.getBody(), RemotingHelper.DEFAULT_CHARSET);
                } catch (UnsupportedEncodingException e) {
                    e.printStackTrace();
                }
                if (s.equals("error")) {
                    System.err.println("这是事务回滚");
                }
                return ConsumeConcurrentlyStatus.CONSUME_SUCCESS;
            }
        }
